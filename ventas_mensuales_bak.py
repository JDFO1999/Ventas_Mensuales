...
def obtener_sucursales(cursor):
    cursor.execute("""
        SELECT cCodigo, cNombre, cRutaIP, cCia
        FROM Sucursal
...
def listar_pos(codigo, ruta_ip, modo):
    if modo == "S":
    ...
    try:
        for entry in os.listdir(path):
            ...
            if os.path.isdir(full) and entry.startswith("POS"):
                try:
                    pos_num = int(entry[3:])
...
def generar_excel(resultados, year, month, output_path):
    ...
    column_widths = {
        "B": 9.5, "C": 12.63, "D": 18.38, "E": 14.13,
        "F": 16.13, "G": 20.38, "H": 16.38, "I": 18.25,
        "J": 17.13, "K": 14.63,
    }
    ...
    for res in resultados:
        ...
        for col_idx, value in enumerate(data, start=1):
            ...
            cell.border = thin_blue_top()

        ws.cell(row=row, column=2).border = thin_black_left()
        ws.cell(row=row, column=11).border = thin_black_right()
...
def main():
    ...
    for idx, sucursal in enumerate(sucursales):
        ...
        pos_dirs, error_listado = listar_pos(codigo, ruta_ip, modo)
        if not pos_dirs:
